// scripts/template-service/qa-computed-diff.mjs
//
// MYGRATR-TEMPLATE-SERVICE — QA computed-value diff.
//
// Extracts getComputedStyle() from the "Assembled desktop" export frame of
// docs/raw-html/Service.html (unpacked from the bundled JSON string) and from
// the built page (/services/ai-consulting on a local `next start`), diffs a
// curated paired element set property-by-property against QA tolerances, runs
// the 5 structural-gate checks (build green, Service+BreadcrumbList JSON-LD,
// both locale routes 200, meta description, no client-render bailout), and the
// chrome/breadcrumb rows (full chrome QA — Service is a chrome-QA template).
//
// Read-only QA tooling. Does NOT edit template code.
//
// Usage: PORT=3120 node scripts/template-service/qa-computed-diff.mjs
...
const EXPORT_SRC = path.join(REPO_ROOT, 'docs/raw-html/Service.html')
const EXPORT_TMP = '/tmp/service-export.html'
const EXPORT_LABEL = 'Assembled desktop'
const PORT = process.env.PORT || '3120'
const BASE = `http://localhost:${PORT}`
const SLUG = 'ai-consulting'
const BUILT_PATH = `/services/${SLUG}`
const BUILT_PATH_UK = `/uk/services/${SLUG}`
const OUT_JSON = path.join(REPO_ROOT, 'audit-output/service/qa-diff-run.json')
...
const FORBIDDEN = new Set(['VideoObject', 'Review', 'Person', 'CollectionPage', 'BlogPosting', 'Article'])
...
async function runGate(buildOk) {
  ...
  const service = ld.filter((b) => b['@type'] === 'Service')
  const bc = ld.filter((b) => b['@type'] === 'BreadcrumbList')
  const forbidden = types.filter((t) => FORBIDDEN.has(t))
  const notes = []
  let ok = true
  if (service.length !== 1) { ok = false; notes.push(`expected 1 Service, found ${service.length}`) }
  else if (!service[0].name || !service[0].url) { ok = false; notes.push('Service missing name/url') }
  ...
  gate.push({ check: 'Service + BreadcrumbList JSON-LD valid, no forbidden types', status: ok ? 'PASS' : 'FAIL', evidence: `@types: [${types.join(', ')}]${notes.length ? ' | ' + notes.join('; ') : ''}` })
