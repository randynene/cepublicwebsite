...
export async function generateStaticParams(): Promise<RouteParams[]> {
  ...
  if (slugs.length < 2) {
    console.error(
      `[TEMPLATE-TOOL] generateStaticParams (UK locale): ${slugs.length} routable paths (expected 2)`,
    )
  }
  return slugs
}
...
export default async function ToolUkPage({
  params,
}: {
  params: Promise<RouteParams>
}) {
  ...
  return (
    <main id="main">
      <ToolJsonLd tool={tool} locale="en-GB" />
      <ToolTemplate tool={tool} locale="en-GB" />
    </main>
  )
}
