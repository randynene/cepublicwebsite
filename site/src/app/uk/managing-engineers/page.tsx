...
// /managing-engineers  (UK mirror - byte-identical per spec §1)
//
...
export async function generateMetadata({
  ...
  return buildHubMetadata('managingEngineersHub', await searchParams, 'en-GB')
}

export default async function ManagingEngineersHubUkPage({
  searchParams,
}: {
  searchParams: SearchParams
}) {
  const data = await resolveBlogHubRoute('managingEngineersHub', await searchParams, 'en-GB')
