...
// /hiring-tips  (UK mirror - byte-identical per spec §1)
//
...
export async function generateMetadata({
  ...
  return buildHubMetadata('hiringTipsHub', await searchParams, 'en-GB')
}

export default async function HiringTipsHubUkPage({
  searchParams,
}: {
  searchParams: SearchParams
}) {
  const data = await resolveBlogHubRoute('hiringTipsHub', await searchParams, 'en-GB')
