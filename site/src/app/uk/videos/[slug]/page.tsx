...
export async function generateStaticParams(): Promise<RouteParams[]> {
  ...
  if (slugs.length < 22) {
    console.error(
      `[TEMPLATE-VIDEO] generateStaticParams (UK locale): ${slugs.length} routable paths (expected 22)`,
    )
  }
  return slugs
}
...
export default async function VideoUkPage({
  params,
}: {
  params: Promise<RouteParams>
}) {
  ...
  return (
    <main id="main">
      <VideoJsonLd video={video} locale="en-GB" />
      <VideoTemplate video={video} locale="en-GB" />
    </main>
  )
}
