...
// /blog  (UK mirror - byte-identical per spec §1)
//
...
export async function generateMetadata({
  ...
  return buildHubMetadata('blogHub', await searchParams, 'en-GB')
}

export default async function BlogHubUkPage({
  searchParams,
}: {
  searchParams: SearchParams
}) {
  const data = await resolveBlogHubRoute('blogHub', await searchParams, 'en-GB')
