...
// /scaling-teams  (UK mirror - byte-identical per spec §1)
//
...
export async function generateMetadata({
  ...
  return buildHubMetadata('scalingTeamsHub', await searchParams, 'en-GB')
}

export default async function ScalingTeamsHubUkPage({
  searchParams,
}: {
  searchParams: SearchParams
}) {
  const data = await resolveBlogHubRoute('scalingTeamsHub', await searchParams, 'en-GB')
