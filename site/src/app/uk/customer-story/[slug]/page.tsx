...
export async function generateStaticParams(): Promise<RouteParams[]> {
  ...
  if (slugs.length < 17) {
    console.error(
      `[TEMPLATE-CUSTOMER-STORY] generateStaticParams (UK locale): ${slugs.length} routable paths (expected 17)`,
    )
  }
  return slugs
}
...
export default async function CustomerStoryUkPage({
  params,
}: {
  params: Promise<RouteParams>
}) {
  ...
  return (
    <main id="main">
      <CustomerStoryJsonLd story={story} locale="en-GB" />
      <CustomerStoryTemplate story={story} locale="en-GB" />
    </main>
  )
}
