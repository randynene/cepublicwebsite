...
export async function generateStaticParams(): Promise<RouteParams[]> {
  ...
  if (slugs.length < 6) {
    console.error(
      `[TEMPLATE-BOOK_A_CALL] generateStaticParams (UK locale): ${slugs.length} routable paths (expected 6)`,
    )
  }
  return slugs
}
...
export default async function BookACallUkPage({
  ...
  return (
    <main id="main">
      <BookACallJsonLd page={page} locale="en-GB" />
      <BookACallTemplate page={page} locale="en-GB" />
    </main>
  )
}
