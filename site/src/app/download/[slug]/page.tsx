...
export async function generateStaticParams(): Promise<RouteParams[]> {
  ...
  if (slugs.length < 2) {
    console.error(
      `[TEMPLATE-DOWNLOAD] generateStaticParams (default locale): ${slugs.length} routable paths (expected 2)`,
    )
  }
  return slugs
}
...
export default async function DownloadDefaultPage({
  params,
}: {
  params: Promise<RouteParams>
}) {
  ...
  return (
    <main id="main">
      <DownloadJsonLd download={download} locale="en-US" />
      <DownloadTemplate download={download} locale="en-US" />
    </main>
  )
}
