...
interface HomeJsonLdProps {
  ...
  faqItems?: readonly HomeFaqEntry[]
}

// Home page structured data. The root layout already emits the sitewide
// Organization + WebSite nodes (Tech Debt #47), so Home adds a WebPage node
// (isPartOf the WebSite) plus a FAQPage node built from the real, answered
// FAQ now stored in Sanity. Both nodes ship in one @graph.
export function HomeJsonLd({ locale, title, description, faqItems }: HomeJsonLdProps) {
