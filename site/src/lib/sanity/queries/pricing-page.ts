...
*[_id == "pricingPage"][0]{
  ...
  testimonial{ eyebrow, titleLead, titleAccent, quote, stats[]{ value, label }, name, role, caption, companyLogo, videoStill },
  faq{
...
export function toPricingContent(doc: PricingPageDoc): PricingContent {
  ...
  const merge = <K extends keyof PricingContent>(key: K): PricingContent[K] =>
    (d[key] ? { ...base[key], ...(d[key] as object) } : base[key]) as PricingContent[K]
  ...
  return {
    hero: merge('hero'),
    calculator: { ...base.calculator, ...(calcDoc as object), model },
