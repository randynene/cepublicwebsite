...
| 57 | Visual editing enablement (Jul 2026) | ~~**`reveal` scroll-animation hydration mismatch.**~~ **RESOLVED 20 Aug 2026.** `use-in-view.ts`'s initial state used to depend on `typeof IntersectionObserver`, which is `undefined` during SSR (so the server rendered `is-in-view` present) but defined in real browsers (so the first client render omitted it) — a guaranteed mismatch on every load, not just under Presentation. Fixed exactly per the original recommendation: `useState` now starts deterministically `false` on both server and first client render (confirmed as a visual no-op — `globals.css`'s `html.motion-ready .reveal` gate only applies post-hydration), and the no-IntersectionObserver fallback flips to visible inside the existing effect instead of at init (one justified `eslint-disable-next-line react-hooks/set-state-in-effect`, lint-clean). Contained to `site/src/components/motion/use-in-view.ts`; `reveal.tsx` untouched. | ✅ RESOLVED |
...
| 59 | Launch / pricing (Jul 2026) | **SUPERSEDED 16 Aug 2026.** `/pricing` now carries its own cost calculator behind an email gate (`templates/pricing/calculator-gate.tsx` + `/api/pricing-unlock`), so the missing-calculator problem this entry described is gone. Two things it raised are still true and now live elsewhere: the gate copy promises an email that nothing sends, and the unlock posts to the QUICK HIRING form guid so a pricing enquiry lands in HubSpot looking like a hiring lead. Both sit with the Lead Agent track. Original entry follows. ~~**`/pricing` does not render the hiring-cost calculator.** On live, `/pricing` embeds `/hiring-cost-calculator` in an iframe. Our `/pricing` is the captured static page (prose only), so the calculator is absent from it. `HiringCostCalculator` is now a real component (`site/src/components/templates/hiring-cost-calculator/calculator.tsx`) - when `/pricing` is redesigned it should render the component DIRECTLY, not iframe our own site to load our own component.~~ | ✅ SUPERSEDED |
...
| 67 | /for-developers mobile fix (13 Aug 2026) | **The For Engineers hero H1 still clips on laptops, live.** At 1440px it reads "Get matched to th" - the compensated H1 needs 686px inside the export's 640px column and `white-space:pre` forbids re-wrapping. Same at 1024 (852px needed) and 1280 (771px); only 1920 and the 768-900 band fit. Below 900px this is resolved (the canvas is abandoned and the H1 takes the sitewide token), so what remains is the 901-1919 band. **Needs a decision, because either fix reverses a deliberate one:** shrink the headline to what the column holds (reverses `0b929ea`, which standardised the marketing hero scale across pages) or let it re-wrap (reverses `1328ee3`, which pinned the written line breaks and stopped "the" orphaning). A third option is widening the 640px hero column, which breaks the 1920 pixel-match against the Figma export. Measurements: `.mobile-qa/widths.mjs` pattern in the fix session. | Design decision (Jake) |
| 68 | /for-developers mobile fix (13 Aug 2026) | **`npm run static:verify-fe2-parity` fails on `main`,** at offset 16703: the "70%" stat body renders `posted.\nWe put you` where the frozen export has `posted. We put you`. Introduced by a CE-45 content edit, not by the mobile fix (verified by stashing). The gate is therefore currently red for everyone, which means it cannot catch a real regression. Either re-freeze the export baseline (`scripts/static/__fe2-original.json`) to accept the intentional newline, or take the newline out of the content. | Next touch of the For Engineers page |
...
| 69 | Sanity wiring audit (16 Aug 2026) | **Two singletons are Studio forms that edit nothing, and both need a decision.** `startHiringPage` backs NO route at all (`/start-hiring` 308s to `/book-a-call`), and `hiringCostCalculatorPage` is 88% empty and ignored by `/hiring-cost-calculator`, which renders from `UI_STRINGS` plus the code rate card (#60). Same shape as #62 and #63: an editor can fill the form and nothing happens. Deliberately NOT unset, because removing fields from a live singleton is a stop-and-ask. Retire all four (these two, `footer.subscribe`, `hireEngineersPage.vet`) in one cleanup pass, or wire them. | Pre-launch schema cleanup (with #62, #63) |
| 70 | Sanity wiring audit (16 Aug 2026) | **The audit method is worth keeping, because the failure it catches is SILENT.** `/contact` read Sanity correctly but the production document was still the old generic captured shape (`eyebrow`/`heroDescription`/`sections`), so every bespoke field came back null and the page rendered its static fallback while Studio appeared to work. Nothing errors, nothing 404s, and the page looks right. Two checks catch it: follow the import graph (not just `page.tsx`) to find routes that never reach `@/lib/sanity`, then run each singleton's REAL GROQ against production and flag results that come back mostly null. Both were run over all 108 routes and all 45 singletons on 16 Aug: 98 routes wired, no second shape mismatch. Genuinely unwired and left that way by choice: `/ask` (unfinished, noindex), `/demo`, the privacy-policy preview harness, `/uk/[...slug]`, and `/hiring-cost-calculator` (#60). | Re-run before any future launch or migration |

| 71 | Marker triage (16 Aug 2026) | **A Sanity-backed change can be committed, merged, deployed, and still do nothing.** Three of eighteen tickets shipped as no-ops and only surfaced because the rendered page was checked against the live dataset. CE-71 reordered `HERO_SLIDESHOW_PROFILES` in code, which reads like the source of truth but is labelled a fallback in `home/content.ts`; the real order lives in `homePage.hero.profiles` and the code edit was inert. CE-63 shipped a correct patch script that was **never run**, so the footer was untouched. CE-70 worked first time because it is static code with no Sanity layer. **Nothing errors in the failing cases** - no 404, no console warning, the page just serves the old data. Same silent-failure family as #70. **The check that catches it:** for any copy or ordering change, query the production dataset for the field and confirm the rendered HTML matches, rather than reading the diff. "The commit exists" is not evidence the change happened. | Standing practice |
| 72 | Marker triage (16 Aug 2026) | **Branch protection on `main` is configured but not enforcing.** A direct `git push origin main` prints `remote: - Changes must be made through a pull request.` and then **succeeds** - the ref updates and Vercel deploys. So the guard rail reads as present in the GitHub settings while stopping nothing, which is worse than having none, because it invites the assumption that a bad push would be blocked. Either enforce it (and adopt PRs) or remove it so the risk is visible. Relevant because an agent shipped the pricing gate to production on 16 Aug against an explicit brief not to. | Repo config decision |
| 73 | Marker triage (16 Aug 2026) | **Agents sharing one checkout corrupt each other's commits, and worktrees are the fix.** Three agents ran in the same folder on 16 Aug, sharing one git index. A customer-story commit (`504f20f`) swallowed 324 lines of another agent's staged pricing work; uncommitted hero and form changes were wiped twice by branch switches and recovered only from ad-hoc backups. **Every parallel lane now runs in its own `git worktree`**, which gives each agent a separate working directory and index. Two costs to know about: a fresh worktree needs `npm install` in BOTH the root and `site/`, and it will not have `site/.env.local` (gitignored), without which `npm run build` fails on `/api/pricing-unlock`. Symlinking `node_modules` from another worktree does NOT work - Turbopack rejects it with "Symlink points out of the filesystem root". | Standing practice |
| 74 | Personal test deploy (20 Aug 2026) | **`SANITY_API_READ_TOKEN` was temporarily made optional in `site/src/lib/env.ts`** (`z.string().min(1)` to `z.string().optional().default('')`), with the matching module-scope guard in `site/src/app/api/draft-mode/enable/route.ts` narrowed to check `!env` instead of `!env?.SANITY_API_READ_TOKEN`. Purpose: unblock a build/deploy with no real Sanity token available yet, on a separate personal Vercel project (`randy12`), not the production `cloud-employee` one. This is safe because it only affects the token's one real consumer: `defineLive`'s `serverToken`/`browserToken` in `live.ts`, which back the Presentation/draft-mode live-preview feature. Ordinary content reads go through `client.ts`'s token-free published-perspective client (confirmed: no `sanityFetch` call site overrides `perspective`), so all 161 files that read from Sanity are unaffected and render real, non-fabricated content with no token at all - nothing was stubbed or faked. The only loss is `/api/draft-mode/enable` (editorial preview), which isn't needed for this deployment. Verified locally: `npm run build` in `site/` exits 0 with `SANITY_API_READ_TOKEN` unset. **Restore `.min(1)` in `env.ts` and the token-emptiness check in `enable/route.ts` once a real token is available for every environment that needs draft-mode preview** - do not let this drift into the permanent state for the production project. | Restore once real token available |

*Last updated: 20 Aug 2026. **The site is LIVE at cloudemployee.io; pushing to `main` deploys to production.***

**MARKER TRIAGE COMPLETE (16 Aug 2026).** All 18 open Marker tickets were worked and shipped to production across `94d24d3`, `69b71e8`, `0e7f5ff` and `f22af79`. Live: CE-17, 58, 59, 60, 61, 62, 63, 65, 66, 67, 68, 69, 70, 71, 72, 73. **Not code, and still open:** CE-64 needs a file-upload field added to the contact form inside the HubSpot portal - that form is rendered by HubSpot in an iframe, so it cannot be added from our side at all. CE-45 was cancelled by Jake.

Three of these deserve a note beyond "done". **CE-68** was not a currency swap: Seb's GBP figures are monthly retainers against a page showing USD day rates, so the unit, the currency and one commitment level all changed together, and the AI tier fell from roughly $36k/mo equivalent to $9,500. Jake confirmed the reduction was intended before it shipped. The panel is now Sanity-backed (`fractionalCtoPage.hero.match`) with a price field per locale, because it being hardcoded is why re-pricing three numbers became a ticket at all. **CE-67** hides the unverified calculators behind a feature flag on all ten pages rather than deleting them; the copy still ships in the page payload. **CE-70** kept its original headshot: the card is an illustrative persona, so the name moved to the Croatian form (Luka) rather than reusing a real roster member's face under a different identity.

**Also shipped in the same window, outside the tickets:** the Travelex wordmark was traced to vector (`scripts/static/build-travelex-wordmark.mjs`) because the asset was 103x22 rendering at 28px and reading blurry on retina; there is no sharper source, so the fix was hard edges rather than more pixels.

**LEAD CAPTURE IS BROKEN AND IS NOT FIXED HERE.** The forms audit (`docs/briefs/active/FORMS-AUDIT.md`) found that the HubSpot workflow which creates deals watches two form ids, `05bfad44` and `4b883c7d` (`/contact`). The quick hiring form - the primary conversion path on nine pages - and the pricing calculator unlock both submit to `8f974ef4`, which matches neither. Those leads become contacts, produce no deal, reach no board, and notify nobody. Separately, `/api/lead` and `/api/pricing-unlock` only post to Slack when HubSpot REJECTS a lead, and the `SLACK_LEADS_WEBHOOK_URL` they depend on is set nowhere - so that safety net has never fired either. HubSpot's own notifications are slow because the workflows carry deliberate waits (15 minutes on the MQL workflow, 4 on book-a-call), which no code change affects. **This is owned by the Lead Agent track (`feat/lead-agent`), not by the triage lane.** A duplicate notifier was written here and deliberately reverted in `f22af79` to avoid double-posting. `SLACK_LEAD_AGENT_TOKEN` is a valid `xoxb` token authenticating as `leads_agent` with **only** `chat:write` - it cannot list channels, and it has not been invited to `#aa-leads-channel` (posting by name returns `channel_not_found`). The channel id is unconfirmed; `docs/lead-journey-map.md` guesses `C074USTDWTC`.

**/for-developers mobile reflow (13 Aug 2026, `b1a9307`, deployed to production).** The For Engineers page is a fixed 1920px Figma frame scaled with CSS `zoom`, and below 900px that had no correct answer: at 393px the factor is 0.2, so body copy rendered at 3px and iOS Safari's text autosizing inflated parts of it back up inside boxes that stayed scaled, producing the text-on-text pile-up; the H1 was separately compensated back up THROUGH the zoom (40px / 0.205 = 195px) and clipped mid-word in its 640px column. Below 900px the canvas is now abandoned rather than tuned - `width:100%`, no zoom, and the export's fixed geometry unpinned so the sections stack (`FE2_MOBILE_MAX` in `index.tsx` and the matching `@media (max-width:900px)` block in `fe2-styles.ts` **must move together**). The reflow is scoped to a `fe2-figma` class on the three lifted-export wrappers, because the React components inside the canvas lay themselves out - the logo marquee stacked vertically when they were not excluded. The CSS deliberately does not depend on the JS effect having run, which also fixes the raw-1920px-frame variant. **Nothing at or above 901px changed** (1920 stays the pixel-match). Two findings left open as Tech Debt #67 (the same H1 clips at 1024-1440 on laptops, needs a design decision) and #68 (the fe2 parity gate is already red on main).

*Prior: 4 Aug 2026 (LAUNCH complete + first post-launch audit). Prior context below describes the pre-launch build.*
