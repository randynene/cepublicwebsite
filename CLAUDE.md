...
| 17 | CONTENT-1D-CLEANUP | 9 other doc types not surveyed for the same migrator-pattern null-literal issue: teamMember, review, bookACall, video, event, tool, download, benefitValue, staffBenefit, blogPost, compareBlog. Inert until Studio surfaces warnings or render-time breaks. Resolution: optional follow-up read-only diagnostic + per-type cleanup if needed. The CONVENTIONS.md "Migrator Field-Write Pattern — Conditional Spread" rule prevents new migrators from reintroducing the bug. | MYGRATR-TEMPLATE-* / pre-launch |
...
*Last updated: May 2026 — MYGRATR-CONTENT-1D-CLEANUP complete (post-phase patch on closed CONTENT-1D). MYGRATR-TEMPLATE-* next.*