...
4. `CONVENTIONS.md` — 7 patterns established through CONTENT-1D-CLEANUP. DESIGN-1 adds 7 more (Token System, Component Specification, Storybook Story, Sanity Client, Sanity Fetch, Render-Discipline, Template Location — see Step 11). Total post-DESIGN-1: 14.
...
2. **Programmatic probe** (run once, record outcome in CAPABILITY_LOG):
   ...
   try {
     await probe.patch('nonexistent-id-probe').set({ name: 'probe' }).commit();
     throw new Error('Token has WRITE permissions — abort DESIGN-1');
   } catch (err: any) {
     if (err.statusCode === 403 || err.statusCode === 401) {
       console.log('✓ Token is read-only (403/401 on write attempt)');
     } else {
       throw new Error(`Unexpected response ${err.statusCode}; manual verification required`);
     }
   }
   ```
...
- `eslint-rules/no-template-literals-in-render.js` — local AST rule for the four evasion patterns `jsx-no-literals` doesn't catch.
...
**5. Test fixtures** — `tests/lint/jsx-no-literals.test.ts` covers all four evasion patterns plus a passing fixture. The Step 10 verifier runs this test as a hard gate.
...
- `site/src/app/page.tsx` — extended to render `homePage.heroHeadline` from seeded singleton via `sanityFetch` (smoke-test prerequisite per C4). **No marketing-copy fallback** (per CMA-Minor-17).
- `scripts/design/seed-hero-headline.ts` — programmatic seed of `homePage.heroHeadline` so smoke test isn't dependent on a manual Studio step (per CMA-Minor-18).
...
**Sub-step 8e — Smoke-test prerequisite: programmatic seed of homepage hero (per C4 + CMA-Minor-17/18):**

Two corrections from v1.2:
...
2. **Seed `heroHeadline` programmatically.** v1.2 said "if empty on the seeded stub, populate via Studio manually." Manual step with no verifier assertion → human error → false-positive smoke test. Add a script that programmatically sets `heroHeadline` to a known placeholder value before smoke test runs.

```ts
// scripts/design/seed-hero-headline.ts
import { createClient } from '@sanity/client';
...
const client = createClient({
  ...
  dataset: process.env.NEXT_PUBLIC_SANITY_DATASET!,
  apiVersion: '2025-01-01',
  token: process.env.SANITY_MIGRATION_WRITE_TOKEN!, // rotated to mygratr-templates-write per Tech Debt #15
  useCdn: false,
});

const PLACEHOLDER = 'DESIGN-1 smoke-test placeholder — replace via Studio';

async function seed() {
  const homePage = await client.fetch(`*[_type == "homePage"][0]{ _id, heroHeadline }`);
  if (!homePage) throw new Error('homePage singleton not found — run SCHEMA-1 seed first');
  if (homePage.heroHeadline) {
    console.log('heroHeadline already populated:', homePage.heroHeadline);
    return;
  }
  await client.patch(homePage._id).set({ heroHeadline: PLACEHOLDER }).commit();
  console.log('Seeded heroHeadline with placeholder');
}
...
Verifier check #20a (new in v1.3): `*[_type == "homePage"][0].heroHeadline` is a non-empty string.
...
2. Run `npx tsx scripts/design/seed-hero-headline.ts`.
...
13a. **JSON ↔ TS sync (new v1.3 per CMA-C1):** `tools/eslint/ui-strings.json` and `site/src/lib/ui-strings.ts` are in sync. Either: (a) hash check — re-run `generate:ui-strings` and assert no diff; or (b) parse both, compare key/value pairs.
...
19b. **Production stega isolation (new v1.3 per CMA-I7):** in a production-build context (`NODE_ENV === 'production' && VERCEL_ENV !== 'preview'`), assert `sanityClient.config().stega?.enabled !== true`. Symmetric safety check protecting against environment-variable misconfiguration leaking stega markers to production.
...
scripts/design/extract-gsap-timings.ts                    (per I4)
scripts/design/generate-ui-strings.ts                     (NEW v1.3 — wired as prebuild per CMA-C1)
scripts/design/seed-hero-headline.ts                      (NEW v1.3 — programmatic seed per CMA-Minor-18)
scripts/design/verify-token-scope.ts                      (NEW v1.3 — viewer-scope probe per CMA-I9)
...
site/package.json                        (Storybook deps + jsx-no-literals + @eslint/compat per CMA-I13)
...
- **v1.3 (2026-05-03 — current):** Cross-model audit (`preset:full`, run 2026-05-03T09-05-40-413-j0a8) findings applied. 3 critical / 11 important / 7 minor / 5 dismissed. All criticals fixed with cascading consequences. **Pending `preset:quick` re-audit, then LOCK.**
- **v1.3-LOCKED (next, after `preset:quick` returns clean):** the version that ships to Claude Code for execution.
...
*End of MYGRATR-DESIGN-1_BRIEF_v1.3.md*
