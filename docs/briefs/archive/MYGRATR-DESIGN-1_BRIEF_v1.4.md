...
| Brief version | v1.4 |
| Status | DRAFT — pending preset:quick re-audit, then LOCK |
...
4. `CONVENTIONS.md` — 7 patterns established through CONTENT-1D-CLEANUP. DESIGN-1 adds 8 more (Token System, Component Specification, Storybook Story, Sanity Client, Sanity Fetch, Render-Discipline, Template Location, F15-Closed — see Step 11). Total post-DESIGN-1: 15.
...
2. **Programmatic probe** (run once, record outcome in CAPABILITY_LOG):
   ```ts
   // scripts/design/verify-token-scope.ts
   ...
   try {
     ...
     // Per F7 — accept any non-2xx as confirmation of read-only scope, with status logged for debugging.
     // Sanity may return 400/409/422 for various rejection reasons; only a successful create() proves write capability.
     console.log('✓ Token rejected write attempt (statusCode:', err.statusCode, '— treating as read-only)');
   }
   ```

If token is write-capable, halt — rotate to a viewer-scoped token before proceeding.

**Sub-step 0d — Storybook ↔ Next.js 16 compatibility probe (per CMA-I11):**
...
- **If `@storybook/nextjs` peerDeps include Next.js 16:** use it.
- **If `@storybook/nextjs-vite` peerDeps include Next.js 16:** use it (preferred — Vite faster than Webpack).
- **If neither supports Next.js 16:** fall back to `@storybook/react-vite` with manual mocking of `next/image`, `next/link`, `next/font` via Storybook decorators. More work but guaranteed to build. Document the chosen adapter and verified Next.js 16 compatibility in CAPABILITY_LOG.

Halt if neither adapter resolves cleanly and `@storybook/react-vite` mocking burns >2 hours — surface to Jake for dev consult under the half-day rule.

**Commit point:** `chore(design-1): branch + scaffold dirs + metadata refresh + token scope check + storybook compat probe`.
...
- `site/src/lib/ui-strings.ts` — generated from the JSON via `scripts/design/generate-ui-strings.ts`. Wired as `prebuild` so it can never drift from the JSON.
- `scripts/design/generate-ui-strings.ts` — generation script. Reads JSON, emits typed TS const.
...
- `eslint-rules/no-template-literals-in-render.js` — local AST rule covering 4 named evasion patterns `jsx-no-literals` doesn't catch (precomputed const, ternary literal arm, template literal in render, helper-wrapped literal). **Coverage scope (per F8):** rule catches the 4 *named* patterns as written; semantically equivalent variants (outer-scope const, destructured alias, helper nested under conditional, reassigned `let`) are NOT guaranteed by this rule. Future hardening: reference-analysis-based rule with broader scope (out of DESIGN-1 scope).
...
- `package.json` — `npm run qa:structural-diff` script (skipped in CI / prepush; manual invoke), `npm run generate:ui-strings` wired as `prebuild`.
...
tools/eslint/ui-strings.json          ← canonical source (Node + TS both read)
            ...
            │
            └──→ scripts/design/generate-ui-strings.ts
                       │
...
**2. Generation script** — `scripts/design/generate-ui-strings.ts`:
...
{
  "scripts": {
    "generate:ui-strings": "tsx scripts/design/generate-ui-strings.ts",
    "prebuild": "npm run generate:ui-strings",
    "build": "next build"
  },
  "devDependencies": {
    "tsx": "^4.x"
...
**Critical (per F13 — 2-model consensus):** `tsx` MUST be in `devDependencies` of the root `package.json`. The `prebuild` step runs in clean CI environments which won't have `tsx` available globally. A missing `tsx` causes the entire Next.js build to fail with no fallback. Verifier check #13b (NEW v1.4): assert `tsx` is listed in root `package.json` `devDependencies`.
...
export default [
  // ... existing config
  {
    ...
    plugins: {
      // fixupPluginRules required: jsx-no-literals may not be ESLint 9 flat-config native (per CMA-I13)
...
**Note on `@eslint/compat`:** `eslint-plugin-jsx-no-literals` is a community plugin and may not have migrated to ESLint 9 flat-config format. `fixupPluginRules()` from `@eslint/compat` adapts legacy plugin rule definitions to flat config (per CMA-I13). If Step 0b determined the project is on ESLint 8 legacy `.eslintrc.cjs`, the import is `plugins: ['jsx-no-literals']` instead and `fixupPluginRules` is unused.
...
**Sub-step 8a — Pre-flight: `next-sanity` package exports probe (per CMA-C3):**

Before any import is written, run the probe to lock the actual import paths the installed version exposes. SCAFFOLD-1 already burned on three brief-mandated paths that didn't exist; v1.0/v1.1/v1.2 were about to burn the same way until cross-model audit caught it.

```bash
cd site
node -e "
  const pkg = require('./node_modules/next-sanity/package.json');
  console.log('Installed:', pkg.name, pkg.version);
  console.log('Exports:', JSON.stringify(Object.keys(pkg.exports || {}), null, 2));
"
```

Decision tree from probe output:

| `pkg.exports` includes | Use |
|---|---|
| `./visual-editing` | `import { VisualEditing } from 'next-sanity/visual-editing'` |
| no `./visual-editing` (v9+) | `import { VisualEditing } from 'next-sanity'` |
| `./live` | `import { defineLive } from 'next-sanity/live'` |
| no `./live` (v9+) | `import { defineLive } from 'next-sanity'` |

Lock the resolved import paths in this brief's working notes and CAPABILITY_LOG before writing any code. Verifier check #19a (new in v1.3) asserts `npm run build` passes — the import-path-drift gate.
...
**Sub-step 8d2 — Symmetric `enable` route hardening (NEW v1.4 per F5):**
...
**Studio Presentation Tool integration:** the `presentationTool({ previewUrl: { previewMode: { enable: '/api/draft-mode/enable' } } })` config in `studio/sanity.config.ts` already uses POST in current `sanity/presentation` versions. Confirm in the smoke test by inspecting Studio's network tab during preview activation; if Studio is sending GET, file an issue against `sanity` and surface to Jake.

Verifier check #19d (NEW v1.4 per F5): grep `enable/route.ts` for `export async function POST` (must exist) and `export async function GET` (must NOT exist). Also assert the secret is read via `request.headers.get(...)` and NOT from `URL.searchParams`.

CONVENTIONS.md F15-Closed pattern updated in v1.4 to cover both `enable` and `disable` symmetrically.
...
1. Run `npm run generate:ui-strings` (Step 6 prerequisite — also wired as `prebuild`).
...
7. **Round-trip B:** edit the field in Studio. Hit publish. Time how long until preview reflects. Record timing.
8. Record both timings in PHASE_HISTORY: "Visual Editing round-trip A measured at Xs; Round-trip B measured at Ys."
9. **Behavioral stega assertion (per CMA-I14):** in the deployed preview HTML, verify stega zero-width markers are present in the rendered `heroHeadline` text. This is the version-invariant authoritative gate; config-shape inspection (verifier check #19) is informational only.

```bash
# Extract preview HTML and check for stega markers (zero-width characters U+200B, U+200C, U+200D, U+FEFF).
# Per F6 — bypass secret read from gitignored file, NOT from env-var expansion in shell history.
# Create ~/.mygratr/vercel-bypass-header.txt containing the single line:
#   x-vercel-protection-bypass: <secret>
# Then:
curl -s -H @~/.mygratr/vercel-bypass-header.txt \
  "https://{preview-url}/?_disableDraftMode=false" \
  | grep -P '[\x{200B}\x{200C}\x{200D}\x{FEFF}]' \
  && echo "✓ Stega markers present" \
  || echo "✗ No stega markers — Visual Editing broken"
```

**Bypass token policy (NEW v1.4 per F6):**
- File at `~/.mygratr/vercel-bypass-header.txt` is gitignored, mode 0600, NOT committed to repo.
- Token rotated post-DESIGN-1 close — track in CLAUDE.md as Tech Debt #19 (rotation cadence).
- Never echo the token in verifier output, even on failure. Verifier wraps the curl in `set +x` if `set -x` is active.
- Never paste the token into messaging tools or CI logs. CI runs MUST use Vercel's own deployment-protection-bypass mechanism (per-build secret), not a long-lived shared token.
- Shell-history exposure: avoid interactive `curl -H "x-vercel-protection-bypass: ..."` patterns. Always read from file via `-H @path`.

**Halt thresholds (resolved per CMA-I12 — §14 wins):**

If round-trip A within 10–20s OR round-trip B within 5–10s: log as risk and continue. QA-1 retests post-template-build.

If round-trip A >20s OR round-trip B >10s: **halt per §14.**
...
2. Re-measure round-trip B with warm CDN (publish twice; measure the second). First publish includes cold CDN propagation.
...
**Capability log draft:** "Visual Editing infrastructure — single client with conditional stega gated on `VERCEL_ENV === 'preview'`, `defineLive({ client, serverToken })` takes one client (NOT client-switching, per CMA-C2), `<VisualEditing>` rendered at layout level (carried from SCAFFOLD-1). Per-template wiring via Sanity Fetch + Render-Discipline conventions in CONVENTIONS.md. Two distinct round-trips: A (click-to-edit) target 10s, B (publish-to-preview) target 5s. Preview URL is the auto-generated Vercel project URL (custom domain deferred to LAUNCH per CMA-I10 Decision A). F15 (POST-only + origin check on draft-mode disable) closed in DESIGN-1 instead of deferred — see CONVENTIONS.md F15-Closed Pattern."
...
13a. **JSON ↔ TS sync (revised v1.4 per F18):** `tools/eslint/ui-strings.json` and `site/src/lib/ui-strings.ts` are in sync. **Preferred:** parse `ui-strings.json` and parse the `UI_STRINGS` object literal from `ui-strings.ts` (via TypeScript AST or regex extraction), then deep-compare key/value pairs. **Alternative:** regenerate TS to a temp string using `generate-ui-strings.ts` and byte-compare to checked-in `site/src/lib/ui-strings.ts` (only valid if generation is byte-deterministic — current implementation is, but fragile). Raw-file hashing of JSON vs TS is meaningless (different formats by design) and explicitly NOT what this check does.
...
17. **Schema-vs-reality findings (tightened v1.3 per CMA-I4):** each REFERENCE.md has the field-to-UI map section AND the schema-vs-reality findings section. **The schema-vs-reality section is non-empty AND each finding has a resolution direction** (one of: schema-relax, template-fallback, data-backfill, deferred-to-STATIC-1, deferred-to-SCHEMA-2). Specifically for `blogPost`, the section MUST contain a finding for `blogPost.author` with resolution direction.
18. **REMOVED in v1.1.**
...
24. `npm run build` passes from repo root (TS + ESLint clean). `prebuild` (which generates `ui-strings.ts` from JSON) passes.
...
[Prose paragraph: what shipped. Token extraction from live + audit cross-reference + GSAP runtime instrumentation shim. ~20 primitives in site/src/components/ui/ with shadcn for forms/overlays + hand-built for brand expression. Tier-1 inventory locked at N components (use actual count from Step 3a). N complex-component specs at docs/design/components/. Storybook deployed to a Vercel project subdomain (deployment-protected). v0.dev prompt template + 3 worked examples. ESLint `jsx-no-literals` + complementary AST rule (`mygratr/no-template-literals-in-render`) configured for site/src/components/templates/** with allowedStrings sourced from tools/eslint/ui-strings.json (canonical SoT, ts generated via prebuild). Playwright structural-diff config skeleton at tools/qa/. Per-template reference docs for 13 confirmed template types (+ TAXONOMY conditional on §12.6); UNKNOWN dropped from set per audit, resolved in docs/templates/_resolved-classifications.md. Visual Editing wired via single sanityClient with conditional stega gated on VERCEL_ENV, defineLive({ client, serverToken }) takes one client (CMA-C2 correction), Sanity Client + Sanity Fetch + Render-Discipline conventions enforce per-template wiring (no wrapper component). F15 (Tech Debt #14) closed: draft-mode disable route is POST-only with origin check. Preview URL is auto-generated Vercel project URL (custom domain deferred to LAUNCH per CMA-I10 Decision A). Both round-trips smoke-tested with behavioral stega assertion: A click-to-edit Xs, B publish-to-preview Ys. Capability log scaffolded with first entry. SANITY_API_READ_TOKEN viewer-scope verified. migrations.metadata.content_phase.content_migrations_rows refreshed from stale 38 to actual 42. migrations.status unchanged at content_complete.]
```
...
**CONVENTIONS.md** — append seven new sections (v1.3 — added Sanity Client Pattern per CMA-I7 + Render-Discipline expansion per CMA-I6 + F15-Closed Pattern per CMA-I8; Sanity Fetch Pattern rewritten per CMA-C2):
...
4. **Sanity Client Pattern (per CMA-I7)** — there is one `sanityClient`. Stega is conditionally enabled on it gated by `VERCEL_ENV === 'preview'`. Never gate stega on multiple environment variables. `sanityClient` in production deployments has stega disabled and ships clean strings (no zero-width markers in HTML). Verifier asserts this with **two checks**: (a) behavioral — preview HTML contains zero-width markers in rendered Sanity strings (verifier #19); (b) production isolation — in production-build context, `sanityClient.config().stega?.enabled !== true` (verifier #19b). The behavioral check is authoritative; config-shape inspection is informational only.
...
6. **Render-Discipline Pattern (expanded per CMA-I6 — four explicit sub-rules):**
...
8. **F15-Closed Pattern (new per CMA-I8) — draft-mode disable route hardening.** SCAFFOLD-1 deferred F15 (POST-only + origin check on `/api/draft-mode/disable`) to TEMPLATE-* / pre-launch. DESIGN-1 closes F15 in Step 8. The disable route exports POST only (no GET); accepts requests only from origins matching `NEXT_PUBLIC_SITE_URL` or `NEXT_PUBLIC_SANITY_STUDIO_URL`. Tech Debt #14 (F15) marked CLOSED in CHANGELOG.
...
- Tech Debt: confirm #16 (`customerStory.companyLogo`), #17 (10 doc types null-literal scan) still open. **Log new debt items from v1.4 audit:**
  ...
  - **#20 (GSAP shim v2 hardening per F10/F11/F12):** future productisation pass — pre-assignment call buffering, `ScrollTrigger.create` direct instrumentation, robust serializer with `WeakSet` cycle detection + `NodeList`/`window`/`document` handling. Best-effort shim is sufficient for CE; v2 is for customer-2 productisation.
...
scripts/design/generate-ui-strings.ts                     (NEW v1.3 — wired as prebuild per CMA-C1; mkdir -p guard added v1.4 per F13)
scripts/design/verify-token-scope.ts                      (NEW v1.3 — viewer-scope probe per CMA-I9; F7 cleanup-on-write-success added v1.4)
...
package.json                             (Storybook scripts, design:verify, qa:structural-diff, test:lint, generate:ui-strings (prebuild), seed:hero-headline, verify-token-scope, design:extract-gsap)
site/package.json                        (Storybook deps + jsx-no-literals + @eslint/compat per CMA-I13; tsx in devDependencies per F13)
...
> You are reviewing the use of `eslint-plugin-jsx-no-literals` (configured with `allowedStrings` derived from `tools/eslint/ui-strings.json` — JSON SoT generated to `site/src/lib/ui-strings.ts` via `prebuild` script per CMA-C1) scoped to `site/src/components/templates/**`. Check:
...
> 4. Does the JSON SoT + generation script pattern (`tools/eslint/ui-strings.json` → `scripts/design/generate-ui-strings.ts` → `site/src/lib/ui-strings.ts` via `prebuild`) close the gap that v1.2's `Object.values(UI_STRINGS)` TS-import-from-eslint.config.mjs left open? Are the two artefacts (JSON + generated TS) provably in sync at build time? Verifier check #13a precision per F18.
...
**Total: ~20 build days + 1.5 recovery days = ~21.5 working days = ~3 weeks of focused work.** Within Jake-wide tolerance. **Rebaselined progressively:** v1.0 claimed 14 days (too optimistic for dev-light posture); v1.1 corrected to 19.5 build / 21 total; v1.3 adds 0.5 day on Step 8 for CMA-mandated probe + F15 closure + behavioral assertion work, bringing total to ~21.5 working days.
...
4. Visual Editing smoke test in Step 8: round-trip A (click-to-edit) exceeds 20s OR round-trip B (publish-to-preview) exceeds 10s. **§14 supersedes §7C** (per CMA-I12 resolution): if within 2× target log as risk and continue per §7C; if exceeds 2× target halt per §14 and apply Step 8g diagnostic checklist.
...
- **v1.4 (2026-05-03 — current):** Second cross-model audit (`preset:full` with 5 diverse-provider panels, run 2026-05-03T09-43-24-392-fkhg) findings applied. 3 critical / 11 important / 6 minor / 7 dismissed. All 9 v1.3-introduced architectural patterns held cleanly under audit; findings were integration defects on the new patterns, not architectural flaws. v1.4 corrections (11 fixes + 5 stale-reference cleanups) are surgical. **Pending `preset:quick` re-audit, then LOCK.**
- **v1.4-LOCKED (next, after `preset:quick` returns clean):** the version that ships to Claude Code for execution.
- Brief deviations during execution → tracked as DEV-N entries appended to this brief, surfaced in PHASE_HISTORY.

After v1.4 lock, this brief moves from `docs/briefs/active/` to `docs/briefs/archive/` at phase close.

---

*End of MYGRATR-DESIGN-1_BRIEF_v1.4.md*
