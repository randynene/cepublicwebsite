...
> `Schedule a Call` - the header CTA opens the booking canvas in place rather than
> routing to `/book-a-call`, and `Back to site` exists because stripping the nav
> otherwise leaves the page a dead end); no sitewide nav, announcement bar or
