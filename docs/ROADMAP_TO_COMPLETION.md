...
|---|---|---|---|---|---|
| Home | `/` | No (redesigned) | WIRED-FALLBACK | Phase 6: fix crash + fill Sanity + kill fallback. **3 Aug polish:** "→ All case studies" under Salmon/Marcus testimonial → `/customer-stories`. **3 Aug photos:** hero rotation expanded 4 → 6 cards (Kyla, Marcelo, Petra, Grace, Gabriel, Russel; Rafael dropped - his source photo has no headroom, see the note on his roster entry). The where-we-work hub panels now use real CE office photos (`site/public/design/home/hubs/`, cropped to survive both the narrow rest state and the wide hover state) instead of Picsum placeholders; uploaded as Sanity assets so Seb can swap any panel. | 5 |
| How It Works | `/how-it-works` | No (redesigned) | WIRED-FALLBACK | Phase 6: fill Sanity + kill fallback. **3 Aug polish:** Stage 3 faces → Molly (left) + Daniel (right); Welcome slide kept; JPG ~130KB; Sanity Stage 3 image patched. **3 Aug:** the two hero cards are real roster engineers (Ivana M. + Stephen L.) pulled from `TALENT_ROSTER` rather than the invented Ana/Reinaldo placeholders; `howItWorksPage.hero.people` patched with their portraits. | 5 |
...
| Book a call (index) | `/book-a-call` | Yes (in sitemap) | **DONE as a booking-only landing (3 Aug).** Jake's call: the page is header + Calendly + footer, nothing else. `StaticPageTemplate` gained a `calendlyOnly` mode - eyebrow, H1, hero copy and body sections are dropped (a visually-hidden H1 stays for SEO/a11y), and the footer "Ready to hire" band plus the Clara bar are suppressed. Empty Sanity content is therefore no longer a gap. Calendly theme is dark navy `#101B30` with `text_color` `#6F7F99`, the one value that clears ~4:1 on both the dark panel and Calendly's hard-coded white inputs. | none (was Phase 1.3) | 4 |
...
>
> **SITEWIDE CTA RENAME (29 Jul).** "Schedule a Call" becomes **"Talk to a human"**
> everywhere. Code occurrences are done (the `/ask` header and its in-page booking
> CTAs, the location-page static fallback, the service/technology detail pages, the
> seed fallbacks). **Four Sanity fields still need it and are Jake's to run:**
> `navigation.ctaButton.label` (the sitewide header CTA) plus
> `start.cards[start-1].cta` on the three `locationPage` docs. Dry-run and apply via
> `npm run static:patch-cta-talk-to-a-human`; `npm run static:scan-schedule-a-call`
> regenerates the full picture. **Deliberately excluded pending a decision:** 6
> `bookACall.metaTitle` values and `bookACallPage.title` (page titles on indexed
> pages - renaming those mid-migration is the trap the hub H1s were kept out of, see
> Tech Debt #43b), and 10 portable-text runs inside blogPost / compareBlog /
> customerStory bodies (migrated article copy held for live parity).
>
> **Complete:** routes `/ask` + `/uk/ask`; bespoke chrome (logo, `Back to site`,
> `Talk to a human` - the header CTA opens the booking canvas in place rather than
> routing to `/book-a-call`, and `Back to site` exists because stripping the nav
> otherwise leaves the page a dead end); the two CTAs are an equal-size pair carrying
> the same `.cta-sweep` hover language as the sitewide header CTA, verified to cause
> no layout shift on hover and no horizontal overflow from 360px up; no sitewide nav,
> announcement bar or
> footer, so the shell claims the whole viewport; the floating Clara widget
> suppressed on this page only, with all analytics still loading (`/ask` IS Clara, so
> a second floating Clara chat on the same screen was a contradiction); the chat
> column (thread, composer, voice
