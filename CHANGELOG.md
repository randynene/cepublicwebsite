...
## MYGRATR-LAUNCH — cutover to cloudemployee.io, and the post-launch audit (3-4 Aug 2026)

**The site is live.** cloudemployee.io now serves the Next.js + Sanity build. DNS moved off Webflow at Cloudflare (www CNAME + apex, both un-proxied because Vercel requires the orange cloud off for its own records), Let's Encrypt certificates issued for both hostnames, and all four entry points funnel to `https://www.cloudemployee.io`: http/https on apex and www all 308 to the one canonical origin. `robots.txt` flipped from `Disallow: /` to `Allow: /` on the first request to the new hostname with no redeploy, because the host-aware rewrite landed first (see below). The 653-URL sitemap is live and was already registered in Search Console.

**Four things shipped in the last hours before cutover, each closing a real gap.** (1) **Host-aware robots** (`0d67ae4`): `robots.ts` read `NEXT_PUBLIC_CANONICAL_HOST` at BUILD time, so the moment that variable was set for production, staging.jakevibes.dev - which IS this project's Vercel production deployment - would have started serving `Allow: /` and published a second indexable copy of the site. It now reads the serving host from request headers, so indexing follows the hostname actually being served rather than a build-time guess. (2) **Geotargetly restored**: the new site carried one rule with a hardcoded `refurl`; live ran three independent rules, each with its own id, timestamp-derived callback name and query-string shape. All three are now reproduced verbatim, because the loader calls `georedirect<TIMESTAMP>loaded` by name and a tidied snippet silently stops firing. (3) **Visitor country served from Vercel**: `window.VISITOR_COUNTRY` came from a Cloudflare Worker reading `CF-IPCountry`, and that Worker only runs on PROXIED traffic, so it dies the moment Vercel's records go grey-cloud. Replaced with Vercel's native `x-vercel-ip-country`, which also gates Hotjar to US/GB exactly as live did. (4) **Meta titles stored verbatim** (`c9fab0b`, `8be3172`): `resolvePageTitle` was appending the brand to any title lacking it, making exact parity with live impossible; Sanity values are now absolute, with doubled suffixes collapsed rather than shipped.

**A self-inflicted outage worth recording.** A Vercel firewall rule challenging Philippines / China / Russia / Singapore was added shortly before cutover, mirroring the old Cloudflare WAF config. It challenged every request from those countries on every page (HTTP 429, `x-vercel-mitigated: challenge`), which locked out Cloud Employee's entire Philippines delivery team and every Filipino visitor - the exact audience a company that recruits Filipino engineers cannot afford to block. Caught from a Slack screenshot roughly 25 minutes in, diagnosed from the served HTML (`<title>Vercel Security Checkpoint</title>`), fixed by disabling the rule. **The lesson is not "firewalls are risky", it is that a security config does not port between platforms without being re-checked against who the audience actually is.** The country list was copied, not reasoned about.

**Post-launch audit (4 Aug), and it is good news.** Search Console and Ahrefs are both wired properly for the first time: the GSC service account (`gsc-reader@`) has full access to `sc-domain:cloudemployee.io`, a DOMAIN property that spans www, non-www and every subdomain and therefore holds continuous history straight through the cutover with no old/new property split. Ahrefs v3 works and **contradicts Tech Debt #4** - the plan does cover this domain (DR 36). Three scripts now make the audit repeatable: `scripts/seo/gsc-pull.ts`, `scripts/seo/verify-gsc-urls.ts` and `scripts/seo/ahrefs-pull.ts`. Replaying all 330 URLs Google has impressions for against production returns **309 clean and zero regressions**: every exception is either faithful to what Webflow already did (verified against the captured pre-cutover behaviour, including `/team/shawnee-malesich` which CE had already removed) or belongs to the separate `talent.` subdomain. **The migration cost nothing.** What the audit did surface is older: 24 URLs holding live backlinks from sites at DR 91, 75 and 73 return 404, and 23 of the 24 were already dead before cutover - inherited link rot, and a reclaim opportunity rather than a repair. And the strategic picture, which no amount of technical work changes: 514,635 impressions per 90 days converting to 1,616 clicks (0.31% CTR), roughly half of them brand searches, leaving genuine non-brand organic at about nine clicks a day. Full findings and the prioritised list in `docs/seo/POST_LAUNCH_AUDIT.md`.
